...
import {
  ShieldCheck, Lock, Mail, ArrowRight, RefreshCw, FileText,
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-24 px-4 text-center">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-1">Failed to load policy</h3>
      ...
      <section className="bg-gradient-to-br from-gray-50 to-white border-b border-gray-100">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12 sm:py-16 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-sm text-gray-400 mt-2 max-w-md mx-auto">How we collect, use, and protect your personal information.</p>
            {policy && (
              <div className="flex items-center justify-center gap-2 mt-4 text-xs text-gray-400">
                <span className="flex items-center gap-1"><FileText size={12} /> Last updated:</span>
                <span className="font-medium text-gray-600">{policy.lastUpdated}</span>
              </div>
            )}
          </motion.div>
        </div>
      ...
      {policy && (
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
          ...
          <motion.div
            ...
            className="flex items-center gap-3 p-4 bg-emerald-50 border border-emerald-200 rounded-2xl mb-10"
          >
            <div className="w-10 h-10 bg-emerald-100 rounded-xl flex items-center justify-center flex-shrink-0">
              <Lock size={18} className="text-emerald-600" />
            </div>
            <div>
              <p className="text-sm font-semibold text-emerald-800">Your data is protected</p>
              <p className="text-xs text-emerald-600 mt-0.5">We use enterprise-grade encryption and follow global data protection standards.</p>
            </div>
          </motion.div>

          {/* Navigation */}
          <nav className="mb-10 p-4 bg-gray-50 rounded-2xl border border-gray-100" aria-label="Policy sections">
          ...
          >
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-white">Privacy Questions?</h2>
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 mt-6">
              <a href={`mailto:${policy.contactEmail}`} className="px-6 py-3 bg-emerald-500 text-white rounded-full text-sm font-semibold hover:bg-emerald-400 transition-all flex items-center gap-2">
                <Mail size={15} /> {policy.contactEmail}
              </a>
